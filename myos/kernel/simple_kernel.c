...
void cmd_help() {
    ...
    set_text_color(BLUE_ON_BLACK);
    print_string("NTFS文件系统命令（持久存储）:\n");
    set_text_color(WHITE_ON_BLACK);
    print_string("  ntfs_format   - 格式化NTFS分区\n");
    print_string("  ntfs_mount    - 挂载NTFS文件系统\n");
    print_string("  ntfs_unmount  - 卸载NTFS文件系统\n");
    print_string("  ntfs_ls       - 列出NTFS目录内容\n");
    print_string("  ntfs_cat      - 显示NTFS文件内容\n");
    print_string("  ntfs_write    - 写入NTFS文件\n");
    print_string("  ntfs_rm       - 删除NTFS文件\n");
}
...
// NTFS文件系统命令
void cmd_ntfs_format() {
    set_text_color(RED_ON_BLACK);
    print_string("警告: 这将格式化NTFS分区并删除所有数据！继续? (y/n): ");
    set_text_color(WHITE_ON_BLACK);
    
    read_input();
    
    if(input_buffer[0] == 'y' || input_buffer[0] == 'Y') {
        print_string("格式化NTFS分区中，请稍候...\n");
        ...
        if(result == 0) {
            print_string("NTFS分区格式化成功！\n");
        } else {
            set_text_color(RED_ON_BLACK);
            print_string("NTFS分区格式化失败，错误代码: ");
            print_int(result);
            print_newline();
            set_text_color(WHITE_ON_BLACK);
        }
...
void cmd_ntfs_mount() {
    ...
    if(result != 0) {
        set_text_color(RED_ON_BLACK);
        print_string("初始化NTFS文件系统失败，错误代码: ");
        print_int(result);
        print_newline();
        set_text_color(WHITE_ON_BLACK);
        return;
    ...
    if(result == 0) {
        print_string("NTFS文件系统挂载成功！\n");
    } else {
        set_text_color(RED_ON_BLACK);
        print_string("NTFS文件系统挂载失败，错误代码: ");
        print_int(result);
        print_newline();
        set_text_color(WHITE_ON_BLACK);
    }
...
void cmd_ntfs_unmount() {
    print_string("卸载NTFS文件系统...\n");
    ...
    if(result == 0) {
        print_string("NTFS文件系统已卸载。\n");
    } else {
        set_text_color(RED_ON_BLACK);
        print_string("NTFS文件系统卸载失败，错误代码: ");
        print_int(result);
        print_newline();
        set_text_color(WHITE_ON_BLACK);
    }
...
void cmd_ntfs_ls(const char* path) {
    // 获取NTFS实例
    ntfs_fs_t* fs = get_ntfs_fs();
...
void cmd_ntfs_cat(const char* filename) {
    if(strlen(filename) == 0) {
        set_text_color(RED_ON_BLACK);
        print_string("错误: 缺少文件名\n");
        set_text_color(WHITE_ON_BLACK);
        print_string("用法: ntfs_cat <文件名>\n");
        return;
    }
...
void cmd_ntfs_write(const char* args) {
    ...
    if(strlen(filename) == 0) {
        ...
        print_string("错误: 缺少文件名\n");
        set_text_color(WHITE_ON_BLACK);
        print_string("用法: ntfs_write <文件名> <内容>\n");
        return;
    }
...
void cmd_ntfs_rm(const char* filename) {
    if(strlen(filename) == 0) {
        set_text_color(RED_ON_BLACK);
        print_string("错误: 缺少文件名\n");
        set_text_color(WHITE_ON_BLACK);
        print_string("用法: ntfs_rm <文件名>\n");
        return;
    }
